...
public class Enemy extends LivingEntity {
	...
	public RayCast rayCastForTarget(double size, int bounces, int immuneFrames, int lifeSpan) {
		double angle = Math.PI/20;
		RayCast cast = null;
		for (int i=0; i < 40; i++) {
			RayCast tempCast = new RayCast(level, x+(width/2), y+(height/2), size, size, this);
			if (tempCast.simulateCast(target, Math.cos(i*angle)*10, Math.sin(i*angle)*10, bounces, immuneFrames, lifeSpan))
